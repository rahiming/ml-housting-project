...
def tip(label, text, fill="FFF3CD"):
    ...
    p.paragraph_format.space_after = Pt(4)
    r1 = p.add_run(label + "  ")
    set_font(r1, bold=True, color=(0x85, 0x63, 0x04) if fill == "FFF3CD" else (0x0C, 0x54, 0x6D))
    r2 = p.add_run(text)
...
toc = [
    ...
    ("Partie 3", "Intégrer MLFlow dans le pipeline principal (src/training/pipeline.py)"),
...
vocab = [
    ("Experiment", "Groupe de runs liés à un même objectif. Ex : \"housing_price_prediction\"."),
    ("Run",        "Une exécution unique du pipeline. Chaque python main.py crée un run."),
    ("Parameter",  "Valeur fixée avant l'entraînement. Ex : n_estimators=100, random_state=42."),
    ("Metric",     "Valeur mesurée après l'entraînement. Ex : mae=0.45, rmse=0.62, r2=0.81."),
    ("Artifact",   "Fichier produit par le run. Ex : le modèle .joblib, le fichier de métriques JSON."),
...
bullet("Les métriques sont dans artifacts/metrics/metrics_v1.json, metrics_v2.json, etc.")
...
bullet("Chaque run est enregistré automatiquement avec ses paramètres, métriques et modèle.")
...
bullet("Colonne gauche : la liste des expériences (housing_price_prediction, housing_ab_models).")
...
numbered("MLFlow affiche un graphique en barres et un tableau côte-à-côte des métriques.")
...
numbered("Le dossier metrics/ contient le fichier .json copié par mlflow.log_artifact().")
...
numbered("Dans l'interface MLFlow UI (http://127.0.0.1:5000), cliquer sur l'onglet « Models ».")
...
files_summary = [
    ("requirements.txt",               "Ajout de mlflow==2.22.0",
     "Installer la dépendance"),
    (".gitignore",                      "Ajout de mlruns/ et mlartifacts/",
     "Ne pas commiter la BDD MLFlow locale"),
    ("src/training/pipeline.py",        "Import mlflow + set_experiment + start_run + log_param/metric/model",
     "Traçabilité du pipeline principal"),
    ("notebooks/02_train_ab_models.ipynb", "Import mlflow + set_experiment + start_run dans cellules 4 et 5",
...
bullet("backend/requirements.txt — MLFlow n'est pas nécessaire dans le conteneur de production.")
